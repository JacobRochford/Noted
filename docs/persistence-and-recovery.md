...
| **Automated fault tests** - verifies expected behavior under controlled file failures | The project includes 69 service-level tests covering Settings recovery, shared atomic writes, locked files, rolling backups, corrupt files, Scratchpad and MiniPad recovery, note drafts, editor sessions, saved-note history, atomic note creation, and full-backup creation, replacement, and storage compatibility. |
