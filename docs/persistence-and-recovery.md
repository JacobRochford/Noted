...
| **User backup** - keeps a verified copy of important Noted data under the user's control | The user creates or updates this backup from Settings. Noted builds and verifies the replacement before switching folders, then keeps the prior user backup as an additional previous copy. If an existing backup is damaged or contains unexpected files, later backup work stops and leaves it unchanged for review. |
...
| **Automated fault tests** - verifies expected behavior under controlled file failures | The project includes 94 service-level tests covering Settings recovery, shared atomic writes, locked files, rolling backups, corrupt files, Scratchpad and MiniPad recovery, note drafts, editor sessions, saved-note history, atomic note creation, and full-backup creation, preview, replacement, and storage compatibility. |
