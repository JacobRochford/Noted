...
public sealed class ScratchpadContentServiceTests
{
    ...
    public void DuplicateContentDoesNotRotateRollingBackup()
    {
        ...
        service.SaveContent("same content");
        service.SaveContent("same content");
    ...
    public void MissingPrimaryRestoresRollingBackup()
    {
        using var directory = new TemporaryTestDirectory();
        var service = new ScratchpadContentService(directory.Path);
        service.SaveContent("previous content");
        service.SaveContent("current content");
        File.Delete(directory.File("scratchpad.rtf"));

        var loadResult = new ScratchpadContentService(directory.Path).LoadContent();
    ...
    public void LockedBackupWarnsWithoutTurningVerifiedSaveIntoFailure()
    {
        using var directory = new TemporaryTestDirectory();
        var service = new ScratchpadContentService(directory.Path);
        service.SaveContent("one");
        service.SaveContent("two");
        ...
        {
            result = service.SaveContent("three");
        }
        ...
        Assert.AreEqual(
            "three",
            new ScratchpadContentService(directory.Path).LoadContent().Content);
